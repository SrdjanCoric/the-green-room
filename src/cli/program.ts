...
import type { listReports } from '../mastra/reporting';
...
/**
 * Everything the commands reach outside their own process: the workflow handle, the
 * session runners, and the stored-state readers. The entry point wires the real ones;
 * tests inject fakes and drive the parser.
 */
export interface CliDeps {
  workflow: () => InterviewWorkflowHandle;
  runInterview: typeof runInterview;
  reconnectInterview: typeof reconnectInterview;
  regradeSession: typeof regradeSession;
  recoachSession: typeof recoachSession;
  loadLastRun: typeof loadLastRun;
...
/** Assemble the interview-coach command tree over the injected dependencies. */
export function buildProgram(deps: CliDeps): Command {
    ...
    .action(
      async (options: {
        ...
        try {
          ...
          preparing = true;
          const { runId, result } = await deps.runInterview({
  ...
  program
    ...
    .action(async (options: { run?: string }) => {
      ...
      const runId = options.run ?? (await deps.loadLastRun())?.runId;
      if (!runId) {
      ...
      try {
        const outcome = await deps.reconnectInterview({
          workflow: deps.workflow(),
          runId,
          ...terminalPrompts(),
        });
  ...
  async function replayReport(
    ...
    const runId = session ?? (await deps.loadLastRun())?.runId;
    if (!runId) {
    ...
    try {
      const requestContext = buildModelRequestContext(resolveModelTiers(options));
      const outcome = await runner({ workflow: deps.workflow(), runId, requestContext });
  ...
  program
    ...
    .action((session: string | undefined, options: ReplayOptions) =>
      replayReport(deps.regradeSession, session, options, {
        verb: 're-grade',
        gerund: 're-grading',
        done: 'Re-graded',
      }),
  ...
  program
    ...
    .option('--smart-model <id>', 'model id for the smart tier (grader, coach)')
    .action((session: string | undefined, options: ReplayOptions) =>
      replayReport(deps.recoachSession, session, options, {
        verb: 're-coach',
        gerund: 're-coaching',
        done: 'Re-coached',
      }),
