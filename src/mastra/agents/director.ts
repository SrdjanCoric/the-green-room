...
- Watch the trend on the current thread: when answers are getting shorter and more mechanical - process and tooling description rather than decisions and outcomes - the thread is dry, and one more follow-up will only get a thinner answer. There is always a hook left in the last answer; a chaseable detail is not by itself a reason to stay. Spend the next question on a new topic instead. The assessment notes say so outright when they see this: a note that the thread is dry means the topic is done even though it never reached enough signal, and the only moves left on it are a new topic or wrapping up.
